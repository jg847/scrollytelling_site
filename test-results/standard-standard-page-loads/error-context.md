...
- Name: standard.spec.ts >> standard page loads
- Location: tests/browser/standard.spec.ts:3:5
...
```ts
  ...
  2 | 
  3 | test("standard page loads", async ({ page }) => {
  4 |   await page.goto("/getting-started/");
> 5 |   await expect(page.getByRole("heading", { level: 1 }).first()).toHaveText("Getting Started");
    |                                                                 ^ Error: expect(locator).toHaveText(expected) failed
  6 | });
  7 | 
```